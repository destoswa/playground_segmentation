...
def dice_loss(logits, targets, eps=1e-6):
    ...
    # probs = logits
    targets_onehot = F.one_hot(targets, num_classes=2).permute(0, 3, 1, 2)
...
class ImageAdaptiveFusion(torch.nn.Module):
    """
    Image-wise adaptive fusion using global features.
    Parameters:
        num_scales (int);
        num_channels (int).
    Returns:
        torch.Tensor - fused logits (B, C, H, W).
    """

    def __init__(self, num_scales, num_channels):
        super().__init__()
        self.pool = torch.nn.AdaptiveAvgPool2d(1)
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(num_scales * num_channels, 64),
            torch.nn.ReLU(),
            torch.nn.Linear(64, num_scales)
        )

    def forward(self, logits_list):
        B, C, _, _ = logits_list[0].shape

        # extract global descriptors
        feats = [self.pool(l).view(B, C) for l in logits_list]
        feats = torch.cat(feats, dim=1)  # (B, S*C)

        weights = self.mlp(feats)        # (B, S)
        weights = torch.softmax(weights, dim=1)

        fused = 0
        for i, logits in enumerate(logits_list):
            fused = fused + logits * weights[:, i][:, None, None, None]

        return fused, weights
    

# class ImageBasedFusion(nn.Module):
#     """
#     Predict global fusion weights from input images using a small CNN.

#     Parameters:
#         in_channels (int) - number of input image channels (e.g. 3 for RGB, >3 for multispectral);
#         num_scales (int) - number of scales to fuse.

#     Returns:
#         torch.Tensor - normalized weights of shape (B, num_scales).
#     """

#     def __init__(self, in_channels=3, num_scales=4):
#         super().__init__()
...
#         """
#         x = self.cnn(pixel_values)              # (B, 64, H/8, W/8)
#         x = self.pool(x).view(x.size(0), -1)    # (B, 64)

#         weights = self.mlp(x)                   # (B, num_scales)
...
class MultiScaleFusionModel(nn.Module):
    ...
    #     self.num_classes = segformer.config.num_labels

    #     self.fusion = ImageBasedFusion(
    #         in_channels=segformer.config.num_channels,
    ...
    #     # ===============================
    #     # 1) Multi-scale inference (frozen)
    #     # ===============================
    #     with torch.no_grad():
    ...
    #     # shape: (B, K, C, H, W)
    #     # ===============================
    #     stacked_logits = torch.stack(logits_per_scale, dim=1)

    #     # ===============================
    #     # 3) Predict weights from image
    #     # shape: (B, K)
    #     # ===============================
    #     weights = self.fusion(pixel_values)

    #     # reshape for broadcasting → (B, K, 1, 1, 1)
    #     weights = weights[:, :, None, None, None]

    #     # ===============================
    #     # 4) Weighted fusion
    #     # ===============================
    #     fused_logits = (stacked_logits * weights).sum(dim=1)  # (B, C, H, W)
    #     # ===============================
    #     # 5) Output
    #     # ===============================
    ...
    def __init__(self, segformer, scales, device=None):
        ...
        # )
        # self.fusion = ImageAdaptiveFusion(
    ...
    def forward(self, pixel_values, labels=None, return_weights=False):
        ...
        with torch.no_grad():  # freeze segmentation
            for s in self.scales:
                ...
                # logits_per_scale.append(logits_resized[:,1,...][:,torch.newaxis,:,:])
                logits_per_scale.append(logits_resized)

                # for i, logits in enumerate(logits_per_scale):
                #     print(i, logits.mean().item(), logits.std().item())
        ...
        # # 6️⃣ Fuse
        # fused_logits, weights = self.fusion(logits_per_scale)
        # # probs = [torch.softmax(l, dim=1)[:, 1:2] for l in logits_per_scale]
        # # fused_logits, weights = self.fusion(probs)
        # print(weights)

        weights = self.fusion(pixel_values)
        # output = torch.sum(stacked_logits * weights[:,:,None,None], dim=1)
        fused_logits = torch.cat([
            torch.sum(stacked_logits[:,0::2,...] * weights[:,:,None,None], dim=1)[:,None,:,:],
            torch.sum(stacked_logits[:,1::2,...] * weights[:,:,None,None], dim=1)[:,None,:,:],
            ], dim=1)
        print(weights)
