...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.processor is not None:
            ...
            imgs = (imgs - mean) / std
            # imgs = np.divide(imgs - mean, std + 1e-6)
            # print(imgs.shape)
            # print(np.mean(imgs, axis=(1,2)).shape)
            # print(np.mean(imgs, axis=(0,1)).shape)
            # print(np.mean(imgs, axis=0).shape)
            # quit()
            # print((imgs - np.mean(imgs, axis=(1,2))).shape)
            # print(np.std(imgs, axis=(1,2)).shape)
            # print((imgs - np.mean(imgs, axis=(1,2))) / np.std(imgs).shape)
            # print('---')
            # imgs = (imgs - np.mean(imgs, axis=(1,2))) / np.std(imgs)
            # print(imgs.shape)
            # quit()
            inputs = {
